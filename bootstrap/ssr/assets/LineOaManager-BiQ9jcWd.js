      ...
      _push(ssrRenderComponent(_sfc_main$1, null, {
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            ...
            _push2(`<!--]--></div></div><button${ssrIncludeBooleanAttr(sending.value || selectedUserIds.value.length === 0) ? " disabled" : ""} class="btn btn-success btn-sm w-full text-white"${_scopeId}>`);
            if (sending.value) {
              _push2(`<span class="loading loading-spinner loading-xs"${_scopeId}></span>`);
            } else {
              _push2(`<!---->`);
