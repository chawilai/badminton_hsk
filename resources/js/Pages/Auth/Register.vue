...
<template>
    <GuestLayout>
        ...
        <div class="text-center mb-6">
            ...
            <h1 class="text-2xl font-bold text-base-content m-0">สมัครสมาชิก</h1>
            <p class="text-sm text-base-content/60 mt-1 m-0">Badminton Party</p>
        </div>

        <form @submit.prevent="submit" class="space-y-4">
            <div>
                <label for="name" class="block text-sm font-medium text-base-content/80 mb-1">Name</label>
                <input id="name" type="text" v-model="form.name" required autofocus autocomplete="name" placeholder="Your name"
            ...
            <p class="text-center text-sm text-base-content/60 m-0">
                มีบัญชีอยู่แล้ว?
                <Link :href="route('login')" class="text-court-600 hover:text-court-700 font-medium no-underline">เข้าสู่ระบบ</Link>
            </p>
        </form>
    </GuestLayout>
</template>
